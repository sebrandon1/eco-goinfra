...
type ResourceCRUD interface {
	...
	Create(namespace, name string, obj interface{}) error
	Update(namespace, name string, obj interface{}) error
...
func (r *AppsV1ResourceCRUD) Exists(namespace, name string) bool {
	_, err := r.Client.Deployments(namespace).Get(context.TODO(), name, metav1.GetOptions{})
	return err == nil
}

func (r *AppsV1ResourceCRUD) Create(namespace, name string, obj interface{}) error {
	switch resource := obj.(type) {
	case *appsv1.Deployment:
		glog.V(100).Infof("Creating deployment %s in namespace %s", name, namespace)
		_, err := r.Client.Deployments(namespace).Create(context.TODO(), resource, metav1.CreateOptions{})
		return err
	case *appsv1.DaemonSet:
		glog.V(100).Infof("Creating daemonset %s in namespace %s", name, namespace)
		_, err := r.Client.DaemonSets(namespace).Create(context.TODO(), resource, metav1.CreateOptions{})
		return err
	case *appsv1.StatefulSet:
		glog.V(100).Infof("Creating statefulset %s in namespace %s", name, namespace)
		_, err := r.Client.StatefulSets(namespace).Create(context.TODO(), resource, metav1.CreateOptions{})
		return err
	case *appsv1.ReplicaSet:
		glog.V(100).Infof("Creating replicaset %s in namespace %s", name, namespace)
		_, err := r.Client.ReplicaSets(namespace).Create(context.TODO(), resource, metav1.CreateOptions{})
		return err
	default:
...
func (r *AppsV1ResourceCRUD) Update(namespace, name string, obj interface{}) error {
	switch resource := obj.(type) {
	case *appsv1.Deployment:
		glog.V(100).Infof("Updating deployment %s in namespace %s", name, namespace)
		_, err := r.Client.Deployments(namespace).Update(context.TODO(), resource, metav1.UpdateOptions{})
		return err
	case *appsv1.DaemonSet:
		glog.V(100).Infof("Updating daemonset %s in namespace %s", name, namespace)
		_, err := r.Client.DaemonSets(namespace).Update(context.TODO(), resource, metav1.UpdateOptions{})
		return err
	case *appsv1.StatefulSet:
		glog.V(100).Infof("Updating statefulset %s in namespace %s", name, namespace)
		_, err := r.Client.StatefulSets(namespace).Update(context.TODO(), resource, metav1.UpdateOptions{})
		return err
	case *appsv1.ReplicaSet:
		glog.V(100).Infof("Updating replicaset %s in namespace %s", name, namespace)
		_, err := r.Client.ReplicaSets(namespace).Update(context.TODO(), resource, metav1.UpdateOptions{})
		return err
	default:
		return errors.New("unsupported resource type")
	}
